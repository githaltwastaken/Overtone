...
| **P-1** | **Sound events from the map** | every object expanded into the sounds it makes: circle; slider head, each repeat, tail (per-edge sound and set), body; spinner end; mania hold. Each resolved to what osu! plays: object sample overriding the timing point's, 0 meaning "inherit". Everything below reads hitsounds through this | H1, H2, H3, H5, P-5 | M | todo |
| **P-2** | **Hitsound field writer** | changes only `hitSound`, `edgeSounds`, `edgeSets` and `hitSample` on hit object lines, as edits over the original bytes. A write with zero changes gives the identical file (a test, and a measurement over every local map). Green lines only with consent, reusing inject's rules | H1, H5 | M | todo |
| **P-3** | **Sample playback** | samples found as osu! finds them (beatmap folder custom index, then skin, then defaults), decoded in the page, scheduled on the existing clock. Defaults are **Overtone's own synthesised set**: osu!'s default samples are ppy's, not ours to bundle | H2, H5 | M | todo |
